...
// Mock quiz data if database has not been seeded yet
const FALLBACK_QUIZ = {
  id: "quiz-math-10",
...
export default function StudentQuizzesPage() {
  const [selectedQuiz, setSelectedQuiz] = useState<typeof FALLBACK_QUIZ | null>(null);
  ...
  const handleStartQuiz = () => {
    setSelectedQuiz(FALLBACK_QUIZ);
    setTimeLeft(FALLBACK_QUIZ.duration * 60);
    setAnswers({});
    setQuizResult(null);
    setQuizStarted(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Call secure server action to grade and save submission
      ...
      if (response.success && response.data) {
        ...
        // Fallback grading logic if server action database fails (e.g. no DB connected)
        let score = 0;
        const correctAnswersMock: Record<string, string> = { q1: "0", q2: "1" };
        selectedQuiz.questions.forEach((q) => {
          if (answers[q.id] === correctAnswersMock[q.id]) {
            score += q.score;
          }
        });
    ...
    <div className="max-w-[800px] mx-auto flex flex-col gap-8">
      ...
      {!quizStarted && (
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-canvas border border-hairline rounded-lg p-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            <div className="flex flex-col gap-2">
              ...
              <p className="font-caption text-ink-muted-80 text-xs max-w-[500px]">
                {FALLBACK_QUIZ.description}
              </p>
              <div className="flex items-center gap-4 text-[11px] text-ink-muted-48 mt-2">
                <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {FALLBACK_QUIZ.duration} phút</span>
                <span>•</span>
                <span>Điểm đạt: {FALLBACK_QUIZ.passingScore.toFixed(1)} / 10</span>
              </div>
            </div>
