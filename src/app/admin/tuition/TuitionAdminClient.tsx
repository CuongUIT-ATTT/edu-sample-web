...
export default function TuitionAdminClient({ classes, initialPrice }: Props) {
  ...
  const now = new Date();
  const [fromMonth, setFromMonth] = useState(7);
  const [toMonth, setToMonth] = useState(7);
        ...
        {classes.map(cls => (
          <div key={cls.id} className="flex flex-wrap items-center justify-between px-5 py-4 border-b border-divider-soft last:border-0 hover:bg-surface-pearl/50 transition-colors gap-3">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[11px] text-ink-muted-48">({cls._count.students} HS)</span>
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <button onClick={() => router.push(`/admin/tuition/${cls.id}?from=${fromMonth}&to=${toMonth}&year=${year}`)}
