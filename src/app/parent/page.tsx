    ...
    <div className="flex flex-col gap-8 max-w-[1200px]">
      ...
      <div className="bg-gradient-to-r from-blue-900 to-indigo-900 border border-indigo-950 rounded-lg p-5 text-white flex flex-col md:flex-row justify-between items-center gap-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="font-tagline text-sm font-bold">
              Kỳ thi Tốt nghiệp THPT 2027 của con đang cận kề
            </h4>
            <p className="text-[11px] text-indigo-200">
              Đồng hành cùng con ôn tập, khắc phục lỗi sai lý thuyết đạt kết quả
              tốt nhất
            </p>
          </div>
        </div>
      ...
      <div className="flex justify-between items-start gap-4 flex-wrap">
        <div>
          ...
          <p className="font-caption text-ink-muted-80 mt-1">
            Phụ huynh học viên: <strong>{childName}</strong> (Lớp {className}).
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        >
          ...
          <div>
            ...
            <h3 className="font-body-strong text-lg font-bold text-ink mt-2 group-hover:text-purple-600 transition-colors">
              {displayTeacherName}
            </h3>
            <p className="text-xs text-ink-muted-48 mt-0.5">
              Hotline: {displayTeacherPhone}
            </p>
          </div>
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Link
          href="/parent/attendance"
          className="bg-canvas border border-hairline rounded-lg p-6 hover:border-green-600 transition-all duration-200 cursor-pointer block"
        >
          <h3 className="font-body-strong text-lg font-semibold text-ink border-b border-divider-soft pb-4 mb-4 flex justify-between items-center">
            <span>Nhật ký điểm danh ca học gần đây</span>
            <span className="text-xs text-green-600 font-semibold hover:underline">
              Chi tiết chuyên cần &rarr;
            </span>
          </h3>
