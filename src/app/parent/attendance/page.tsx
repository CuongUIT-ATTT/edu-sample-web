...
export default async function ParentAttendancePage() {
  ...
  try {
    if (session) {
      ...
      const firstChild = parentProfile?.students[0];
      if (firstChild) {
        childName = firstChild.user.name;
        attendances = firstChild.attendances.map((a) => ({
          id: a.id,
  ...
  const statusConfig: Record<string, { label: string; classes: string }> = {
    PRESENT: { label: "Có mặt", classes: "bg-green-50 text-green-700 border border-green-200" },
    ABSENT: { label: "Vắng mặt", classes: "bg-red-50 text-red-700 border border-red-200" },
    LATE: { label: "Đi trễ", classes: "bg-yellow-50 text-yellow-700 border border-yellow-200" },
    EXCUSED: { label: "Có phép", classes: "bg-blue-50 text-blue-700 border border-blue-200" },
  };
  ...
  return (
    <div className="flex flex-col gap-8">
      {/* Header */}
      <div>
        <h1 className="font-tagline text-2xl font-semibold text-ink">Theo dõi chuyên cần của con</h1>
        <p className="font-caption text-ink-muted-80 mt-1">
          Lịch sử chuyên cần chi tiết của học sinh: <span className="font-semibold text-ink">{childName}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <div className="bg-canvas border border-hairline rounded-lg p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-caption-strong text-ink-muted-48 uppercase font-bold">Có mặt</span>
          </div>
        ...
        <div className="bg-canvas border border-hairline rounded-lg p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-caption-strong text-ink-muted-48 uppercase font-bold">Vắng mặt</span>
          </div>
        ...
        <div className="bg-canvas border border-hairline rounded-lg p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-caption-strong text-ink-muted-48 uppercase font-bold">Đi trễ/Phép</span>
          </div>
        ...
        <div className="bg-canvas border border-hairline rounded-lg p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-caption-strong text-ink-muted-48 uppercase font-bold">Tỉ lệ đi học</span>
          </div>
      ...
      <div className="bg-canvas border border-hairline rounded-lg shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-hairline bg-surface-pearl">
          <h2 className="font-body-strong text-sm text-ink">Bản ghi điểm danh gần đây của con</h2>
        </div>
        {attendances.length === 0 ? (
          <div className="p-16 text-center">
            <CheckSquare className="h-12 w-12 text-ink-muted-48 mx-auto mb-4" />
            <p className="font-body text-ink-muted-80">Chưa ghi nhận điểm danh.</p>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-hairline">
                ...
                <th className="text-center px-6 py-3 text-[11px] font-caption-strong text-ink-muted-48 uppercase tracking-wider">Trạng thái</th>
                <th className="text-left px-6 py-3 text-[11px] font-caption-strong text-ink-muted-48 uppercase tracking-wider">Nhận xét từ giáo viên</th>
              </tr>
            </thead>
            <tbody>
              {attendances.map((a) => (
                <tr key={a.id} className="border-b border-hairline last:border-0 hover:bg-surface-pearl transition-colors">
                  ...
                  <td className="px-6 py-3 text-sm text-ink-muted-80 font-caption">{a.remarks || "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
