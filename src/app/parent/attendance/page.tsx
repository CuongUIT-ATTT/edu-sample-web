    ...
    if (session) {
      ...
      if (firstChild) {
        ...
        attendances = firstChild.attendances.map((a) => ({
          id: a.id,
          date: a.date.toLocaleDateString("vi-VN", {
            weekday: "long",
            year: "numeric",
            month: "long",
            day: "numeric",
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <h1 className="font-tagline text-2xl font-semibold text-ink">
          Theo dõi chuyên cần của con
        </h1>
        <p className="font-caption text-ink-muted-80 mt-1">
          Lịch sử chuyên cần chi tiết của học sinh:{" "}
          <span className="font-semibold text-ink">{childName}</span>
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-canvas border border-hairline rounded-lg p-5 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] font-caption-strong text-ink-muted-48 uppercase font-bold">
              Có mặt
            </span>
          </div>
        ...
        <div className="bg-canvas border border-hairline rounded-lg p-5 shadow-sm">
          ...
          <p className="text-3xl font-bold font-tagline text-red-600">
            {absentCount}
          </p>
        </div>
        <div className="bg-canvas border border-hairline rounded-lg p-5 shadow-sm">
          ...
          <p className="text-3xl font-bold font-tagline text-yellow-600">
        ...
        <div className="bg-canvas border border-hairline rounded-lg p-5 shadow-sm">
          ...
          <p
            className={`text-3xl font-bold font-tagline ${attendanceRate >= 90 ? "text-green-600" : attendanceRate >= 75 ? "text-yellow-600" : "text-red-600"}`}
          >
      ...
      <div className="bg-canvas border border-hairline rounded-lg shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-hairline bg-surface-pearl">
          <h2 className="font-body-strong text-sm text-ink">
            Bản ghi điểm danh gần đây của con
          </h2>
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {attendances.map((a) => (
                ...
                >
                  <td className="px-6 py-3 text-sm text-ink font-caption">
                    {a.date}
                  </td>
                  <td className="px-6 py-3 text-center">
                    <span
                      className={`text-xs font-caption-strong px-3 py-1 rounded-full ${statusConfig[a.status]?.classes}`}
                    >
                      {statusConfig[a.status]?.label || a.status}
                    </span>
                  </td>
                  <td className="px-6 py-3 text-sm text-ink-muted-80 font-caption">
                    {a.remarks || "—"}
                  </td>
                </tr>
              ))}
