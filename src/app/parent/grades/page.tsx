...
import { TrendingUp, Award, BookOpen, BarChart2, Trophy } from "lucide-react";
  ...
  try {
    if (session) {
      ...
      if (firstChild) {
        ...
        for (const g of firstChild.grades) {
        ...
        if (gradesBySubject.length > 0) {
          overallGpa =
            Math.round(
              (gradesBySubject.reduce((sum, s) => sum + s.gpa, 0) /
    ...
    console.error("Error fetching parent child grades:", error);
  }
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="font-tagline text-2xl font-semibold text-ink">
            Báo cáo điểm thi thử của con
          </h1>
          <p className="font-caption text-ink-muted-80 mt-1">
            Bảng điểm thi thử và luyện đề chi tiết của học viên:{" "}
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2 bg-purple-50 border border-purple-200 rounded-lg px-4 py-2.5 shadow-sm">
            ...
            <div>
              <p className="text-[9px] font-caption text-ink-muted-48 uppercase tracking-wider">
                Danh hiệu tích lũy
              </p>
          ...
          <div className="flex items-center gap-3 bg-canvas border border-hairline rounded-lg px-5 py-3 shadow-sm">
            ...
            <div>
              <p className="text-[10px] font-caption text-ink-muted-48 uppercase tracking-wider">
                Điểm trung bình (GPA)
              </p>
              <p
                className={`text-2xl font-bold font-tagline ${overallGpa >= 8 ? "text-green-600" : overallGpa >= 6.5 ? "text-blue-600" : "text-red-600"}`}
              >
      ...
      {gradesBySubject.length === 0 ? (
        <div className="bg-canvas border border-hairline rounded-lg p-16 text-center shadow-sm">
          ...
          <p className="font-body text-ink-muted-80">
            Chưa có kết quả học tập được ghi nhận cho con của bạn.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-hairline bg-surface-pearl">
                <div className="flex items-center gap-3">
                  <span className="text-[10px] font-bold bg-purple-600 text-white px-2 py-0.5 rounded-full uppercase">
                    {subject.subjectCode}
                  </span>
                  <h3 className="font-body-strong text-sm text-ink">
                    {subject.subjectName}
                  </h3>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <div
                    className={`text-sm font-bold font-tagline px-3 py-1 rounded border ${getScoreColor(subject.gpa)}`}
                  >
                    TB Môn: {subject.gpa}
                  </div>
                </div>
                  ...
                  {subject.grades.map((g, i) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-3.5 text-center">
                        <span
                          className={`text-sm font-bold px-3 py-1 rounded-lg border ${getScoreColor(g.score)}`}
                        >
                          {g.score}
                        </span>
                      </td>
                      <td className="px-6 py-3.5 text-center text-xs text-ink-muted-80 font-caption">
                        Lý thuyết: {(g.score * 0.6).toFixed(1)} đ • Vận dụng:{" "}
                        {(g.score * 0.4).toFixed(1)} đ
                      </td>
                      <td className="px-6 py-3.5 text-xs text-ink-muted-80 font-caption">
                        {g.date}
                      </td>
                      <td className="px-6 py-3.5 text-xs text-ink-muted-80 font-caption">
                        {g.remarks ||
                          "Làm bài tốt, lưu ý các lỗi lý thuyết cơ bản."}
                      </td>
                    </tr>
                  ))}
                </tbody>
