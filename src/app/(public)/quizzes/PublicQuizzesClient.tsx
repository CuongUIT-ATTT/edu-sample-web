        ...
        {quizStarted && quizResult && selectedQuiz && showReview && quizResult.correctAnswers && (
          <div className="flex flex-col gap-6 max-w-[800px] mx-auto w-full animate-fade-in">
            ...
            <div className="flex flex-col gap-6 mt-4">
              {/* Helper: render a single review question */}
              {(() => {
                    ...
                    <div key={q.id} className="flex flex-col gap-3 border-b border-divider-soft pb-4 last:border-0 last:pb-0">
                      ...
                      {q.type === "MULTIPLE_CHOICE" && (
                        <div className="flex flex-col gap-2">
                          {q.options.map((opt, optIndex) => {
                            ...
                            return (
                              <div key={optIndex} className={`flex items-center gap-3 p-3.5 rounded-pill border text-xs ${btnStyle}`}>
                                <span className={`h-4 w-4 rounded-full border flex items-center justify-center text-[10px] font-bold ${
                          ...
                          {q.options.map((opt, optIndex) => {
                            ...
                            return (
                              <div key={optIndex} className="grid grid-cols-12 items-center gap-2 py-1 text-xs border-b border-divider-soft last:border-0 last:pb-0">
                                ...
                                <div className={`col-span-3 text-center font-bold ${rowCorrect ? "text-green-700" : "text-red-600"}`}>{studVal}</div>
                                <div className="col-span-3 text-center font-bold text-green-700">{corrVal}</div>
                              </div>
                            );
                          })}
                        </div>
                      ...
                      {q.type === "SHORT_ANSWER" && reviewInfo && (
                        <div className="flex flex-col gap-2 bg-surface-pearl/50 p-4 border border-divider-soft rounded-lg text-xs font-body">
                          <div>Bạn đã trả lời: <strong className={isCorrect ? "text-green-700 font-bold" : "text-red-600 font-bold"}>{answers[q.id] || "(Chưa nhập)"}</strong></div>
                          <div>Đáp án đúng: <strong className="text-green-700 font-bold">{reviewInfo.correctAnswer}</strong></div>
                        </div>
                      )}
